...
class AttendanceCorrectionRequestTest extends TestCase
{
    ...
    {
        [$user, $attendanceRecord] = $this->createUserWithAttendance();
    ...
    // new_dateを改変して送信しても、元の勤怠日が申請日付として保存される
    public function test_modified_new_date_is_ignored(): void
    {
        [$user, $attendanceRecord] = $this->createUserWithAttendance();
        ...
        // POSTした2026-10-01ではなく、元の勤怠日が保存される
        $this->assertSame(
            '2026-09-05',
            $application->new_date->toDateString()
        );
    ...
    // 一般ユーザー作成、勤怠作成
    private function createUserWithAttendance(array $overrides = []): array
    {
